...
class PullScheduledFlights extends Command
{
    ...
    public function handle()
    {
        ...
        // Do departures
        $res = $client->request('GET', 'https://flightxml.flightaware.com/json/FlightXML3/AirlineFlightSchedules?start_date=1560636000&end_date=1560654000&origin=KATL&howMany=900', [
            'auth' => [
        ...
        // Do arrivals
        $res = $client->request('GET', 'https://flightxml.flightaware.com/json/FlightXML3/AirlineFlightSchedules?start_date=1560628800&end_date=1560654000&destination=KATL&howMany=900', [
            'auth' => [
                Config::get('flights.username'), Config::get('flights.api_key')
            ]
        ]);
        ...
        foreach($result->AirlineFlightSchedulesResult->flights as $r) {
            ...
            if($flight == null)
                $flight = RwFlight::where('code', $r->actual_ident)->first();
            if($flight == null && $r->arrivaltime >= 1560636000 && $r->arrivaltime <= 1560654000) {
                $flight = new RwFlight();
                $flight->code = $r->ident;
                if($r->actual_ident != '')
                    $flight->code = $r->actual_ident;
